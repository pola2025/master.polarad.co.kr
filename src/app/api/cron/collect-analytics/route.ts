...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // 5. 지역별 통계 수집
    console.log("[Cron] Collecting region stats...");
    try {
      const regionsData = await getRegionData(7);
      const today = new Date().toISOString().split("T")[0];

      const regionsWithDate = regionsData.map((item) => ({
        date: today,
        ...item,
      }));

      const savedCount = await saveRegionStats(regionsWithDate);
      ...
      console.log(`[Cron] Region stats: ${savedCount} records saved`);
    } catch (error) {
      console.error("[Cron] Region stats error:", error);
      results.collections = {
        ...(results.collections as object),
        countries: {
          status: "error",
